...
class ModalManager {
    ...
    calculateVersatility() {
        ...
        // 全能乘区 = 1 + 固定值 + (获取量 / (获取量 + 11200))
        const versatility = 1 + fixed + (acquired / (acquired + 11200));

        // 全能增幅乘区 = 1 + [固定值 + (获取量 / (获取量 + 11200))] × Q
        const versatilityAmplification = 1 + (fixed + (acquired / (acquired + 11200))) * Q;
    ...
    calculateMasteryModal() {
        ...
        // 精通乘区 = 固定值 + (获取量 / (获取量 + 19975))
        const mastery = fixed + (acquired / (acquired + 19975));
        
        // 精通增幅乘区 = 1 + [固定值 + (获取量 / (获取量 + 19975))] × J
    ...
    calculateMasteryTransform() {
        ...
        // 精通乘区 = 固定值 + (获取量 / (获取量 + 19975))
        const masteryZone = fixedValue + (acquired / (acquired + 19975));

        // 精通转化 = [固定值 + (获取量 / (获取量 + 19975))] × Y × 100%
        const masteryTransform = masteryZone * yCoefficient / 1;
    ...
    calculateLuckProbability() {
        ...
        // 幸运概率 = 固定值 + (获取量 / (获取量 + 19975))
        const luckProbability = fixedValue + (acquired / (acquired + 19975));
